...
export async function getMarketWisePlLevelTwo(body = {}, isToast = false) {
  try {
    const callParams = await getAuthCallParams(strings.GET, body, isToast);
    const response = await makeCall(
      `${UrlConstant.MarketWiseLevelTwo}?type=${body.Type}&dataType=${body.dataType}&pageSize=${body.totalEntries}&page=${body.pageNumber}&search=${body.search}`,
      callParams,
      isToast
    );
    return response;
...
export async function getMarketWiseAllUserPlLevelThree(body = {}, isToast = false) {
  try {
    const callParams = await getAuthCallParams(strings.GET, body, isToast);
    const response = await makeCall(
      `${UrlConstant.AllUserMarketwiseLevelThree}/${body.marketId}?pageSize=${body.totalEntries}&dataType=${body.dataType}&page=${body.pageNumber}&search=${body.search}`,
      callParams,
      isToast
    );
    return response;
